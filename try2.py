...
def capture_image():
    ...
    try:
        # Run the command to capture an image
        subprocess.run(['libcamera-still', '-o', str(image_path)], check=True)
...
def handle_motion():
    """Handle the actions when motion is detected."""
    print("Motion Detected!")
    
    # Activate the buzzer
    GPIO.output(BUZZER_PIN, GPIO.HIGH)
    time.sleep(0.1)  # Buzzer ON for a brief moment
...
try:
    while True:
        if GPIO.input(SENSOR_PIN):
            # Run motion handling in a separate thread to ensure immediate response
